...
fn apply(self: *Patcher) void {
    ...
    while (self.patch_idx < self.patch_buf.len - 3) {
        ...
        const record: IpsPatchRecord = .{
            .offset = std.mem.readInt(u24, self.patch_buf[self.patch_idx .. self.patch_idx + 3], .big),
            .length = std.mem.readInt(u16, self.patch_buf[self.patch_idx + 3 .. self.patch_idx + 5], .big),
        };
        ...
        if (record.length > 0) {
            ...
            const rle_length = std.mem.readInt(u16, self.patch_buf[self.patch_idx..(self.patch_idx + 2)], .big);
