...
fn apply(self: *Patcher) void {
    ...
    while (self.patch_idx < self.patch_buf.len - 3) {
        ...
        if (record.length > 0) {
            std.debug.print("record: offset={d}, length={d}\n", .{ record.offset, record.length });
            const replace_len =
            ...
            std.debug.print("record: offset={d}, length={d}, rle length={d}\n", .{ record.offset, record.length, rle_len });
